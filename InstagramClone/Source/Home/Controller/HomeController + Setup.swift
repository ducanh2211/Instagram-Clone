...
extension HomeController {
    ...
    private func setupNavBar() {
        ...
        let image1 = UIImage(named: "message")!
        let messageButton = AttributedButton(image: image1, size: CGSize(width: 22, height: 22))

        let image2 = UIImage(systemName: "heart", withConfiguration: config)!
        let notificationButton = AttributedButton(image: image2)

        navBar = CustomNavigationBar(shouldShowSeparator: false,
                                     leftBarButtons: [logoButton],
                                     rightBarButtons: [messageButton, notificationButton])
    }
    ...
    private func createStorySection() -> NSCollectionLayoutSection {
        ...
        section.contentInsets = NSDirectionalEdgeInsets(top: 0, leading: 14, bottom: 14, trailing: 14)
